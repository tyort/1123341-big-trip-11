...
export default class CardListItemForm extends AbstractSmartComponent {
  constructor(cardItem, allpoints) {
    ...
    this._activateCheckedType = new Map(this._allPoints.map((it) => [it.type, false])).set(cardItem.type, true);
    this._activateCheckedPurpose = new Map(this._allPoints.map((it) => [it.name, false])).set(cardItem.name, true);
